...
The project is initialized and connected to [`Aftikharmnz/USA_Canada_public_data_oil_and_gas_01`](https://github.com/Aftikharmnz/USA_Canada_public_data_oil_and_gas_01). Initial GitHub Pages publication and workflow verification are in progress. The previously exposed EIA key must still be rotated and its replacement added only as the GitHub secret `EIA_API_KEY` before automated EIA refreshes are enabled. Canada needs no provider secret. Local data activation is complete, but the site must not be described as a verified public live service until the first Pages deployment succeeds.
