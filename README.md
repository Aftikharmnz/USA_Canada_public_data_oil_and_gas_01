...
**USA weekly breadth, USA PADD origin-destination movements, and the Canada dashboard are activated, deployed, and verified.** The USA manifest contains 69 active EIA definitions: 66 weekly definitions and three monthly definitions, including monthly crude-oil and total-petroleum-products PADD movements. USA and Canada remain the two primary country pages, with `/reference/` as the educational glossary and `/products/` only a backwards-compatible USA-Refined entry.

The current promoted USA run is `eia-20260805T163902Z`. It contains 217,582 canonical observations, 361 verified observed chart assets, and 361 checksum-linked forecast records (722 integrity entries). Canonical JSON is 90,001,839 bytes (85.83 MiB), below the 90 MiB publication guard. The refresh inserted 74 observations, revised 60, and matched 12,673 unchanged rows. Forecast status is 353 ready, 1 `limited_history`, and 7 unavailable. Every one of the 66 weekly definitions reaches `2026-07-24`, and monthly crude production reaches `2026-05`. Fifteen of 17 crude-movement routes reach `2026-05`; routes 3→5 and 5→3 remain at `2026-04`, so the crude movement manifest conservatively reports `2026-04`. All 18 total-products routes reach `2026-05`. The previous last-known-good generation is `eia-20260729T175851Z`. GitHub Actions run `31026239390` completed successfully and deployed the matching public site.

The current promoted Canada run is `canada-20260803T170245Z`: 69 active definitions (67 Statistics Canada and 2 Canada Energy Regulator), presented as 31 **Crude** and 38 **Refined** choices. It contains 61,310 canonical observations, 467 verified observed chart assets with 467 matching forecast records (934 integrity entries), and 29,739,716 bytes (28.36 MiB) of canonical JSON; public assets occupy 12.78 MiB. The refresh inserted 35 rows, revised 0, and matched 56,335 unchanged rows. Statistics Canada balance tables 25-10-0063-01 and 25-10-0081-01 and movement table 25-10-0077-01 reach source month `2026-05`; CER reaches week `2026-07-21`. Forecast status is 360 ready, 74 `limited_history`, and 33 unavailable. The previous last-known-good generation is `canada-20260731T162758Z`; retention keeps the current and previous generations. Public manifest, asset, integrity, and Pages verification passed. Fifteen table 25-10-0063-01 definitions expose source-published crude grades, bitumen production and processing, equivalent products, and grade-specific refinery inputs; table 25-10-0077-01 adds pipeline-only origin-destination movements for crude/equivalents and combined HGL/RPP scopes.
...
The current USA boundary combines [Phase 4 weekly breadth](docs/phase-4-usa-weekly-breadth.md) with the registered [USA PADD origin-destination movements](docs/usa-padd-movements.md), while [Phase 3 refined products](docs/phase-3-refined-products.md), [Phase 2 USA MVP](docs/phase-2-usa-mvp.md), and [Phase 1](docs/phase-1-scope.md) are historical. [Canada data](docs/canada-data.md) remains the Canada boundary. The secondary `/usa-weekly/` workspace filters the promoted USA manifest rather than duplicating observations or lineage and exposes all 66 registered weekly definitions.
...
The promoted manifest divides its 69 definitions into 12 **Crude** and 57 **Refined** choices. The `/usa-weekly/` desk remains limited to the 66 weekly definitions.
...
See the exact Phase 4 source keys, semantics, and activation evidence in [Phase 4 weekly breadth](docs/phase-4-usa-weekly-breadth.md), the new exact route allowlists in [USA PADD movements](docs/usa-padd-movements.md), and the complete registries and [data catalog](docs/data-catalog.md).
