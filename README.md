...
**The complete 78-definition USA and 96-definition Canada generations are promoted for publication.** Canada now includes 14 CER propane/butane export views and Trans-Northern refined-products throughput, refreshed automatically by the existing Canada workflow. The USA registry remains 66 weekly and 12 monthly definitions, classified as 21 **Crude** and 57 **Refined** choices. The figures below are a verified September 10, 2026 UTC snapshot, not a live freshness guarantee: read each public manifest and chart's source period for current status. Registry activation alone does not prove deployment. USA and Canada remain the primary country pages, with `/reference/` educational and `/products/` a backwards-compatible USA-Refined entry.
...
The active Canada registry contains 96 definitions: 79 Statistics Canada and 17 Canada Energy Regulator, classified as 32 **Crude** and 64 **Refined** choices. New CER export routes distinguish the province of exportation from production and first destination from final consumption. Trans-Northern reports broad refined-product throughput at named pipeline points, not provincial gasoline demand. These are separate source views, never extra terms added to a Statistics Canada balance. See [CER data](docs/cer-data.md). The existing Statistics Canada propane/residual-fuel and pipeline-transporter inventory definitions are unchanged; sibling 25-10-0081 views still share one downloaded cube.

The promoted Canada run is `canada-20260910T055513Z`, with all 96 definitions, 81,123 canonical observations, 695 observed chart assets and 695 matching forecast records (1,390 integrity entries). Canonical JSON is 39,609,838 bytes (37.77 MiB); growth of 4,379,745 bytes (4.18 MiB) passed the unchanged 8 MiB growth allowance. Statistics Canada monthly definitions reach `2026-06`; CER weekly refinery data reaches `2026-08-18`, NGL exports `2026-05`, and Trans-Northern `2026-06`. Forecast status is 483 ready, 90 `limited_history`, and 122 unavailable. Missing current export facts block forecasts from old numeric endpoints. The previous generation `canada-20260910T044955Z` is retained. A repeated live CER refresh returned `changed: false`, with no duplicate generation. Details are in the [CER verification record](docs/audits/2026-09-10-cer-enrichment.md).
