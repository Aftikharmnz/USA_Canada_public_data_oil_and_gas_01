...
**The complete 78-definition USA and 81-definition Canada generations are promoted and deployed.** The USA registry and public manifest both contain 66 weekly and 12 monthly definitions, classified as 21 **Crude** and 57 **Refined** choices; the nine monthly crude-balance definitions have completed promotion. Canada likewise includes its 12-definition expansion. The figures below are a verified September 10, 2026 UTC snapshot, not a live freshness guarantee: read each public manifest and chart's source period for current status. Registry activation alone does not prove deployment. USA and Canada remain the two primary country pages, with `/reference/` as the educational glossary and `/products/` only a backwards-compatible USA-Refined entry.

The promoted USA run is `eia-20260910T044447Z`: 226,306 canonical observations, 414 observed chart assets, and 414 checksum-linked forecast records (828 integrity entries). Sharded canonical JSON occupies 93,907,018 bytes (89.56 MiB), below the 128 MiB logical-generation guard. Forecast status is 397 ready, 1 `limited_history`, and 16 unavailable. Every one of the 66 weekly definitions reaches `2026-08-28`. Monthly crude production and most monthly balance series reach `2026-06`; conservative series-wide latest periods remain `2026-01` for monthly crude exports, `2026-04` for crude PADD movements, and `2026-05` for total-products PADD movements. Individual geography/route dates can differ; a successful refresh does not imply synchronized source releases.

The active Canada registry contains 81 definitions: 79 Statistics Canada and 2 Canada Energy Regulator, classified as 32 **Crude** and 49 **Refined** choices. The 12-definition expansion adds four exact propane measures and six exact residual-fuel-oil measures from the shared table 25-10-0081 cube, plus two month-end pipeline-transporter inventory measures from table 25-10-0075-01. Statistics Canada views 25-10-0081-01 and 25-10-0081-02 are sibling presentations of the same WDS PID `25100081`; they are fetched once and are never treated as independent additive sources. Table 25-10-0075-01 contributes only closing inventories held by domestic pipeline transporters for its two broad product buckets. Those stocks are not total commercial inventory, movement volumes, capacity, or daily rates.

The promoted Canada run is `canada-20260910T044955Z`, containing all 81 definitions (79 Statistics Canada and 2 CER), presented as 32 **Crude** and 49 **Refined** choices. It contains 73,181 canonical observations, 583 observed chart assets with 583 matching forecast records (1,166 integrity entries), and 35,230,093 bytes (33.60 MiB) of sharded canonical JSON. Every Statistics Canada monthly definition reaches source month `2026-06`; CER reaches week `2026-08-18`. Forecast status is 453 ready, 89 `limited_history`, and 41 unavailable. Latest source and latest numeric periods remain distinct where a current Canadian cell is suppressed or unavailable. Retention keeps current and previous generations; failed future refreshes preserve the last-known-good generation.
...
The current USA boundary combines [Phase 4 weekly breadth](docs/phase-4-usa-weekly-breadth.md), the registered [USA PADD origin-destination movements](docs/usa-padd-movements.md), and the active [monthly PADD crude supply/disposition cohort](docs/usa-monthly-crude-balance.md), while [Phase 3 refined products](docs/phase-3-refined-products.md), [Phase 2 USA MVP](docs/phase-2-usa-mvp.md), and [Phase 1](docs/phase-1-scope.md) are historical. [Canada data](docs/canada-data.md) remains the Canada boundary. The secondary `/usa-weekly/` workspace filters the promoted USA manifest rather than duplicating observations or lineage and exposes all 66 registered weekly definitions. Country-local `/usa/profile/` and `/canada/profile/` workspaces put every exact compatible measure in the promoted manifest for one official region and product into a compact grid, with explicit refinery, logistics, related-product, and unavailable-source sections. See [regional profiles](docs/regional-profiles.md).

Refined regional profiles include a period-selectable, unit-aware supply/demand **components** view for the exact chosen product and official region. It is not a forced closed balance: unavailable local demand, movements, adjustments, or processing inputs stay explicit rather than becoming zero or being borrowed from a national/parent-product series. Product supplied remains implied demand, and stock levels remain separate from flows. See the [September 10 reliability and refined-region accuracy audit](docs/audits/2026-09-10-refined-regional-audit.md) for verified coverage, fixes, and remaining source limitations.
...
The browser supports explicit same-level custom regional sums for registry-approved additive quantities, including PADD 1 + PADD 2 and Alberta + Saskatchewan. All nine active monthly USA crude-balance definitions are registered for complete-coverage PADD sums; a computed combination never replaces a source-published U.S. row. The Canada expansion registers nine exact 25-10-0081 measures for complete-coverage province/territory sums: all four propane measures plus residual-fuel net production, imports, exports, stock change, and ending stocks. Residual product supplied remains national-only, and both 25-10-0075 transporter stocks remain source-published only. The browser recomputes seasonal bands, deltas, distributions, and latest-source status from aligned component history; it never adds derived statistics or component prediction-interval endpoints. Bottom-up combined point forecasts require every matching component forecast, and combined prediction intervals require at least 40 exact cross-region residual matches per horizon. Display-unit switching converts compatible barrel and cubic-metre scales without changing source data, and selector labels show both full names and trader-facing abbreviations such as `bbl/d` and `kb/d`. Registered Canadian monthly flow series also offer clearly labelled monthly-average daily-rate scales calculated with each period's actual calendar-day count. Month-end inventories and percentages remain in their valid dimensions. The sticky selection panel can be collapsed to a compact summary while scrolling.

Every registered import series with an official national total and authorized regional components now includes a dedicated composition chart. It keeps the source-published U.S./Canada total as a separate line and shows PADD districts of entry or Canadian provinces as colored stacked bars, with exact-period shares, changes, status, coverage, and a signed reconciliation diagnostic. Missing or suppressed components are never stale-filled or rescaled to 100%, and a rounding gap is never invented as an “Other” region. Canadian pipeline-movement choices use a separate route view with explicit **shipping origin → receiving destination** labels; these pipeline-only corridors are not presented as customs-import contributions.

## Registered USA data

The active USA registry and currently promoted manifest both divide 78 definitions into 21 **Crude** and 57 **Refined** choices. The `/usa-weekly/` desk remains limited to the 66 weekly definitions; the nine promoted monthly crude-balance definitions are available in the explorer and matching regional profiles.
...
| EIA crude supply and disposition | Monthly | PADD | Source-published United States where available; complete-coverage custom PADD combinations are computed views |
| EIA crude and total-products domestic movements | Monthly | Exact shipping PADD → receiving PADD route | None; routes are not aggregated |
...
See the exact Phase 4 source keys, semantics, and activation evidence in [Phase 4 weekly breadth](docs/phase-4-usa-weekly-breadth.md), the exact route allowlists in [USA PADD movements](docs/usa-padd-movements.md), the active [monthly crude balance contract](docs/usa-monthly-crude-balance.md), and the complete registries and [data catalog](docs/data-catalog.md).
...
- Each country has an `Explorer | Regional profile` subnavigation. Profiles use the same promoted manifest/assets, so successful scheduled data refreshes update them automatically without a second data pipeline.
- All analytical views default to a graph-first presentation: chart identity, exact geography, a compact unit picker, and critical data-status notices stay visible, while supporting statistics, tables, descriptions, and methodology open through per-chart **Show details** controls.
- Regional profiles never substitute national values for absent PADD/province values. Adjacent product scopes and refinery/logistics measures are shown in separately labelled context sections.
- The profile Weekly/Monthly control preserves native monthly assets and derives monthly views only for explicitly registered weekly definitions. For the same exact product/geography/measure, an available source-monthly asset wins over the weekly-derived duplicate. Rates are exact calendar-day-weighted averages; stocks, days supply, and utilization are final-weekly snapshots; source-monthly data are never interpolated to weeks.
...
docs/
  ...
  regional-profiles.md Regional profile UI, frequency, and source-boundary contract
...
For USA/EIA only, missing series use registry bootstrap bounds of `2014-01-01` for weekly data and `2014-01` for monthly data. Once USA history exists, routine runs automatically use 13-week weekly and 10-year monthly revision overlaps. Canada instead uses the actual registered source regimes: table 25-10-0063-01 from 2016, the shared 25-10-0081 cube's post-2019 refined-product regime, table 25-10-0075-01 from 2020, and a configured 2014 CER lower bound even though the source file contains older history. Both country publishers skip generation/promotion when observations and relevant metadata are unchanged and retain the current plus previous validated generation after promotion. New canonical schema `1.1.0` generations are deterministically partitioned by series and year: each canonical shard and the revision ledger are capped at 16 MiB, the logical generation at 128 MiB, and refresh growth at the larger of 10% or 8 MiB. The physical candidate is read back before `CURRENT` moves. Legacy schema `1.0.0` single-file generations remain readable during the migration; see [ADR 0007](docs/adr/0007-sharded-canonical-generation-storage.md).
