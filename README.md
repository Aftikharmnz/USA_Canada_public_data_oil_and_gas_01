...
**USA Phase 4 weekly breadth and the Canada dashboard are activated, deployed, and verified.** The USA registry now contains 69 active EIA definitions: the 67 definitions in the promoted Phase 4 generation plus two registered monthly PADD origin-destination movement definitions for crude oil and total petroleum products. Sixty-six are weekly and three are monthly. The movement definitions are eligible for the next validated refresh but are not presented as part of the older promoted generation until its manifest and assets are replaced successfully. USA and Canada remain the two primary country pages, with `/reference/` as the educational glossary and `/products/` only a backwards-compatible USA-Refined entry.

The current promoted USA run is `eia-20260722T202149Z`. It contains the 67 definitions activated in that run, 212,512 canonical observations, 326 verified observed chart assets, and 326 checksum-linked forecast records (652 integrity-checked assets total). Canonical JSON is 87,599,291 bytes (83.54 MiB), below the 90 MiB publication guard. The activation merge inserted 50,435 observations, revised 0, and matched 7,873 unchanged rows. Forecast status is 325 ready, 1 `limited_history`, and 0 unavailable. Every one of the 66 weekly definitions reaches `2026-07-17`; monthly crude production reaches `2026-04`. GitHub Actions run `29954739606` completed successfully and deployed the matching public site.

The current promoted Canada run is `canada-20260728T143908Z`: 69 active definitions (67 Statistics Canada and 2 Canada Energy Regulator), presented as 31 **Crude** and 38 **Refined** choices. It contains 60,814 canonical observations, 467 verified observed chart assets with 467 matching forecast records (934 integrity entries), and 29,518,923 bytes (28.15 MiB) of canonical JSON; public assets occupy 12.67 MiB. The merge inserted 11,088 rows, revised 0, and matched 44,908 unchanged rows. Statistics Canada balance tables 25-10-0063-01 and 25-10-0081-01 reach source month `2026-04`, movement table 25-10-0077-01 reaches `2026-05`, and CER reaches week `2026-06-16`. Forecast status is 360 ready, 74 `limited_history`, and 33 unavailable. The previous last-known-good generation is `canada-20260720T192043Z`; retention keeps the current and previous generations and pruned `analytics-20260720T152511Z`. Public manifest, asset, and integrity verification passed locally. Fifteen table 25-10-0063-01 definitions expose source-published crude grades, bitumen production and processing, equivalent products, and grade-specific refinery inputs; table 25-10-0077-01 adds pipeline-only origin-destination movements for crude/equivalents and combined HGL/RPP scopes.
...
The promoted manifest divides its 67 definitions into 11 **Crude** and 56 **Refined** choices. The registry additionally contains one Crude and one Refined monthly movement choice pending the next successful promotion. The `/usa-weekly/` desk remains limited to the 66 weekly definitions.
