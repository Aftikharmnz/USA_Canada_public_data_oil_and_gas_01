...
The current implementation covers EIA API v2, Statistics Canada full-table delivery, the CER weekly crude-runs file, strict registry/geography normalization, JSON generation storage, revision merging, observed analytics, standalone forecast assets, country manifests, unchanged-poll suppression, and scheduled same-workflow refresh/Pages deployment. Promoted USA run `eia-20260728T164833Z` contains all 69 active definitions (66 weekly and three monthly), including the 28 Phase 4 additions represented by 77 exact source-series keys and two monthly PADD-movement definitions: 217,223 canonical observations, 361 verified observed chart assets, 361 matching forecast records (722 integrity entries), and 89,851,624 bytes (85.69 MiB) of canonical JSON. Its merge inserted 4,711 rows, revised 0, and matched 8,951 unchanged rows; forecast status is 353 ready, 1 `limited_history`, and 7 unavailable. All weekly definitions reach `2026-07-17`; monthly crude production and all 17 crude routes reach `2026-04`. The total-products movement manifest conservatively reports `2026-03`, with 16 route assets at `2026-04` and two at `2026-03`. The previous USA last-known-good generation is `eia-20260722T202149Z`, and public workflow run `30379912166` deployed the matching site. Promoted Canada run `canada-20260728T143908Z` contains all 69 active definitions (67 Statistics Canada and 2 CER), classified as 31 Crude and 38 Refined, with 60,814 canonical observations, 467 verified observed chart assets, 467 matching forecast records (934 integrity entries), and 29,518,923 bytes (28.15 MiB) of canonical JSON. Public assets occupy 12.67 MiB. The merge inserted 11,088 rows, revised 0, and matched 44,908 unchanged rows; forecast status is 360 ready, 74 `limited_history`, and 33 unavailable. Statistics Canada balance tables reach `2026-04`, movement table 25-10-0077-01 reaches `2026-05`, and CER reaches `2026-06-16`. The previous Canada last-known-good generation is `canada-20260720T192043Z`; retention keeps the current and previous generations and pruned `analytics-20260720T152511Z`. Each generated country manifest records its exact forecast readiness summary for that vintage. The verified public site is https://aftikharmnz.github.io/USA_Canada_public_data_oil_and_gas_01/; the replacement EIA secret is configured and the automated workflow has captured provider updates. Durable complete raw-body archival and release-time vintage reconstruction remain target architecture rather than current runtime claims.
...
flowchart TB
    ...
    subgraph Static["GitHub Pages artifact"]
      USA["/usa · promoted 12 Crude / 57 Refined"]
      USAWeekly["/usa-weekly · verified weekly subset"]
      CAN["/canada · 31 Crude / 38 Refined"]
      Reference["/reference"]
...
Canada uses the same generation and public-asset contract in `data/cache/canada` and `public/data/canada`. Its 69-series active registry produces 467 verified observed chart assets with 467 linked forecast records in current run `canada-20260728T143908Z`. Statistics Canada source symbols and missing/suppressed states remain canonical fields; freshness distinguishes the latest source period from the latest numeric period. Table 25-10-0077-01 movements preserve shipping and receiving as separate semantic endpoints and never treat Canada aggregates as additive provincial components. CER national crude runs are derived only from complete same-week coverage of the three registered source regions and retain membership/component lineage. CER utilization remains regional because the source provides no explicit compatible capacity series for a defensible national ratio.
