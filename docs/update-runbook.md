...
`cancel-in-progress: false` protects the running workflow, not an unlimited queue: the current default concurrency queue holds only one pending run. Another USA, Canada, or ordinary Pages run can replace that pending run. Check whether a missed poll was **cancelled while pending** before diagnosing it as a provider or application failure. Later scheduled polls provide another opportunity; if one is needed immediately, dispatch the country workflow on current `main` after the active Pages run finishes. Do not repeatedly re-run older workflow attempts expecting a new commit: they retain their original SHA. A future queue-policy change needs explicit tests for rebasing/validation and stale deployments. [GitHub concurrency semantics](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/control-workflow-concurrency).
...
As with the EIA workflow, a failure before the guarded push leaves the repository and prior Pages artifact unchanged, and a concurrent branch advance is rejected before artifact upload. If a Canada data commit is pushed but deployment fails, rerun manual dispatch with `publish_unchanged: true` so the verified current Canada generation is included in a rebuilt complete site.
...
- The current verified latest week is `2026-07-21`. Record `unknown` freshness until a reliable expected-period rule is reviewed, and do not invent a release timestamp.
...
Future-vintage checks also avoid freezing the count of available route/weekly assets or requiring a forecast to stay available forever. The exact registered series and manifest asset identities must still match. A newly numeric registered Canada pipeline corridor is allowed to appear, while a route that has not reached the newest source month remains a missing exact-period cell rather than being stale-filled. Product, mode, endpoint, source-generation and methodology mismatches still fail closed. Suppression or fewer than 40 cross-component residual matches per horizon withholds the combined projection, not the valid observed history or the entire provider refresh.
