...
A regional import-composition view is authorized only when the manifest measure is `imports`, a source-published national asset exists, and the exact series/component level appears in `config/aggregation/custom-geography.json`. Canada uses mutually exclusive `province_territory` assets and the USA uses mutually exclusive `padd` assets. Statistics Canada's overlapping Atlantic aggregate is excluded; EIA's source-combined PADD 4&5 is used only for the import series that publishes that exact member.

For each source period \(t\), the chart preserves the official national observation \(N_t\) separately from the source-published components \(C_{i,t}\). The displayed share is \(100 C_{i,t}/N_t\) only when the national value and every required component are numeric and \(N_t \ne 0\). The component subtotal is descriptive, while \(N_t-\sum_i C_{i,t}\) is shown as a signed reconciliation/rounding diagnostic only under that same complete coverage. A missing, suppressed, withheld, or unavailable component remains null: the browser withholds that period's stacked composition and shares, does not use an older value, normalize known components to 100%, reconstruct the national total, or create an “Other” geography. Exact known values remain available in the status table. Unit conversion happens only after exact-period alignment; a Canadian monthly-average daily-rate view divides each value by that period's own calendar-day count before applying the selected rate scale.

The import-composition chart is a comparison of published observations, not a new computed observation or geography. USA PADD imports mean district of entry, not foreign origin, destination, or consumption. Canadian crude imports use the documented January 2020+ province-of-destination method; other Canadian balance imports retain their source reporting-province/province-of-entry meaning.
