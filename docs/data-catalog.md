...
| Crude supply and disposition | [`petroleum/sum/snd`](https://www.eia.gov/opendata/browser/petroleum/sum/snd) | Monthly | PADD 1-5 and source-published U.S. where available | Exact source rows; all nine allow complete-coverage, same-level PADD sums, while published U.S. rows stay authoritative | Nine exact definitions active; first complete refresh/promotion pending; see [USA monthly crude balance](usa-monthly-crude-balance.md) |
...
Missing weekly/monthly histories use registry bootstrap bounds of 2014-01-01/2014-01. The historical Phase 3 activation selected its 36 weekly definitions from 2014-01-01 and inserted 130,964 observations. Promoted run `eia-20260805T163902Z` contains 217,582 canonical observations; its legacy schema `1.0.0` canonical JSON is 85.83 MiB. Its manifest references 361 observed chart assets and 361 forecast records and reports `2026-07-24` as the latest period for all 66 weekly definitions. Monthly crude production reaches `2026-05`; 15 of 17 crude movement routes reach `2026-05`, while routes 3→5 and 5→3 remain at `2026-04`; all 18 total-products routes reach `2026-05`. These counts are a verified current record, not constants for future runs. New generations follow the sharded limits in [ADR 0007](adr/0007-sharded-canonical-generation-storage.md).
...
|---|---|---|---|---|---|
| Crude oil and equivalent supply/disposition | [25-10-0063-01](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2510006301) | Monthly | 20 definitions: 5 headline balances plus 15 production-grade, bitumen, equivalent-products, and refinery-input detail rows | Per-series published provinces or Atlantic aggregate plus published Canada where available; prefer the published Canada value and preserve suppression | Active and promoted through source month `2026-05` |
| Petroleum products supply/disposition | [25-10-0081-01](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2510008101) and sibling [25-10-0081-02](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2510008102) | Monthly | 39 definitions in one WDS PID `25100081`: the prior 29 gasoline/ethanol/distillate/jet definitions plus 4 propane and 6 residual-fuel leaves | Per-series published province/territory plus published Canada where available; sibling views are one cube, and availability/suppression vary by exact coordinate | Active registry; 29-definition cohort promoted, 10-definition expansion awaits first complete promotion |
| Inventories held by domestic transporters | [25-10-0075-01](https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=2510007501) | Monthly | 2 closing-inventory definitions: crude/equivalents and combined HGL/RPP, pipeline mode only | Canada plus AB, BC, MB, NT, ON, QC, and SK as exact source rows; published Canada is separate, no custom sum, point stocks are volume-only | Active registry; reviewed cube reached `2026-05`, first complete promotion pending |
...
- Views 25-10-0081-01 and 25-10-0081-02 are sibling presentations of the same WDS PID `25100081`. The refresh downloads that cube once, and definitions from the two views must never be added merely because the web table exposes two presentation URLs.
- The new 25-10-0081 leaves are exact propane and residual-fuel-oil coordinates. Their production, imports, exports, products-supplied, stock-change, and ending-stock rows remain separate measures; parent totals and child products can overlap and are not a stacking authorization.
...
- Table 25-10-0075-01 is a custody-stock table, not a movement or total-commercial-inventory table. The first tranche fixes `Mode of transportation = Pipeline` and `Inventories = Closing inventories` as semantic dimensions and retains only the two exact source product buckets. Across the reviewed numeric history, opening inventory in month *t* duplicates closing inventory in month *t-1*, so opening rows are deliberately not exposed as a second shifted measure.
- The 25-10-0075-01 Canada row remains the authoritative source-published national observation. Provincial rows are not authorized for a custom sum, and the source's 2016 DGUIDs are aliases to the existing stable geography nodes rather than a second geography version. Northwest Territories HGL/RPP cells can be unavailable and must not be zero-filled.
- Custom geography authorization covers all four new propane definitions plus residual-fuel net production, imports, exports, stock change, and ending stocks, always with mutually exclusive same-level members and complete period coverage. Residual-fuel product supplied is national-only; both transporter inventory definitions remain excluded.
- Residual-fuel ending stocks and both transporter closing-inventory definitions are point-in-time volumes. They are excluded from the monthly-average daily-rate registry; the distinct nine-definition rate set is all four propane definitions plus residual-fuel net production, imports, exports, product supplied, and stock change.
...
|---|---|---|---|---|---|
| Refinery crude runs | Weekly observations | Western Canada; Ontario; Quebec & Eastern Canada | Computed Canada reported-region sum only when all three regions exist for the same week | Complete-coverage `sum` with membership and component lineage | Active and verified through `2026-07-21` |
| Refinery utilization | Weekly observations | Same three source-published confidentiality regions | None | `not_aggregatable`; never average percentages | Active regionally and verified through `2026-07-21` |
